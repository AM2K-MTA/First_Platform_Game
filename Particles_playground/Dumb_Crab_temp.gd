...
onready var rc_check_ground = $Ground_detector
onready var rc_floor_detector_L = $FloorDetectorLeft
onready var rc_floor_detector_R = $FloorDetectorRight
...
var move_speed = 3.25 * Globals.UNIT_SIZE		# = 5 * Globals.UNIT_SIZE or = 5 * 32
var gravity
var direction = -1
var anim = ""
...
func _apply_movement():
	print("on Dumb_Crab, direction: " + str(direction))
	print("on Dumb_Crab, move_speed, velocity.x: " + str(move_speed) + ", " + str(velocity.x))
	velocity.y += gravity
	velocity.x = move_speed
	
	if (core_SM.state == core_SM.states.WALKING):
		if not rc_floor_detector_L.is_colliding():
			#print("on Dumb_Crab, 'not rc_floor_detector' LEFT!!")
			velocity.x = 0
			direction *= -1
	...
	if (is_on_floor()):
		#print("on Dumb_Crab, is on floor: " + str(is_on_floor()))
		velocity.y = 0
		is_grounded = true
	else:
		velocity.x = 0

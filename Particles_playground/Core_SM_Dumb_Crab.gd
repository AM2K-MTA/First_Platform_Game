...
func _die():
	#print("On Core_SM_Dumb_Crab, func _die(), BEFORE parent.queue_free(), is parent dead: " + str(parent.is_inside_tree()))
	parent.queue_free()
	#print("On Core_SM_Dumb_Crab, func _die(), AFTER parent.queue_free(), is parent dead: " + str((parent as KinematicBody2D).is_inside_tree()))
	self.queue_free()
	...
	match state:
		states.IDLE:
			...
			if (!parent.is_on_floor() and !parent.is_dragged_by_movable_obj):
				if (parent.movement.y > 0):
					print("On Core_SM_Dumb_Crab, states.IDLE, I'm falling!!!!!!!!!!!!")
		...
		states.WALKING:
			...
			if (parent.is_on_floor()):
				...
				if (parent.movement.y > 0):
					print("On Core_SM_Dumb_Crab, states.WALKING, I'm falling!!!!!!!!!!!!")
					#print("pos: " + str(parent.position))
					return states.FALLING
		states.FALLING:
			#print("On Core_SM_Dumb_Crab, get_transition(), states.FALLING!")
			if (parent.is_on_floor()):
				return states.IDLE
			#pass
		states.DYING:
